   ...
   const startDate = req.body.data?.createdAt?.startDate ?? null;
    ...
    try {
       // const findAllProviders = await Providers.find(filter).skip(index * size).limit(size).sort('-email');
        ...
        const findAllProviders = await Providers.find(filter).populate('user_id').populate('user_id').populate('status_id').populate('services').skip(index * size).limit(size);
        res.status(200).send({
            status: 200,
            index: index,
            size: size,
...
const updateProviders = async (req, res) => {
    try {
        ...
        const findProviders = await Providers.findOne(filter).populate('status_id').populate('services');
